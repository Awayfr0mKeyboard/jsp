...
<body>
	<!-- 
		encType - POST 로 전성되는 데이터의 인코딩 설정
		
		enctype="application/x-www-form-urlencoded"		기본값
		- 전송되는 모든 문자들을 서버로 보내기 전에 인코딩 됨을 암시
		
		encType="text/plain"
		일반 문자 - 공백은 + 기호로 변환하지만 나머지 문자는 인코딩 되지 않음을 명시
	...
	<!-- enctype="text/plain" -->
		...
		취미 : <input type="checkbox" name="hobbies" value="축구" /> 축구
			  <input type="checkbox" name="hobbies" value="datebase" /> database
