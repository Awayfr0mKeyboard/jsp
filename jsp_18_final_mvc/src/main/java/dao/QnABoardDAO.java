...
public class QnABoardDAO {
	...
	private BoardVO getBoardVO(ResultSet rs) throws SQLException {
		...
		board.setQnaReLev(rs.getInt("qnaReLev")); // 답변 글 view level 번호
	...
	public void boardReply(BoardVO vo) {
		...
		try {
			...
			String sql = "INSERT INTO v_qna_board(qnaTitle, qnaContent, qnaWriterNum, qnaReRef, qnaReSeq, qnaReLev) "
					+ " VALUES(?,?,?,?,?,?)";
			pstmt = conn.prepareStatement(sql);
	...
	public ArrayList<BoardVO> getBoardList(Criteria cri) {
		...
		String sql = "SELECT * FROM v_qna_board ORDER BY qnaReRef DESC, qnaReSeq ASC LIMIT ?, ?";
	...
	/**
	 * @param qnaNum - 매개변수로 전달받은 게시글 번호로 게시글의 조회수 1 증가
	 */
