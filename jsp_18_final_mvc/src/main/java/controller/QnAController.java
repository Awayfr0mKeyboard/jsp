...
public class QnAController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("QnAController 요청 처리");
		...
		if (command.equals("boardUpdateFrom.qna")) {
			BoardVO board = service.getBoardByNum(request);
			request.setAttribute("board", board);
		...
		if (command.equals("boardDelete.qna")) {
			...
			if (qnaNum > 0) {
				response.sendRedirect("boardDelete.qna?qnaNum="+qnaNum);
			} else {
				response.sendRedirect("boardList.qna");
			}
			return;
		...
		if (nextPage != null) {
			...
			response.sendError(404, "처리할 수 없는 요청입니다.");
		}
	}
