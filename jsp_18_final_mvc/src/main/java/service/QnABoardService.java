...
public class QnABoardService {
	 ...
	 * @return boolean - 게시글 등록 성공 여부를 boolean type 으로 반환
	 */
	...
	/**
	 * @return - ArrayList<BoardVO> qna_board table 의 전체 게시글 목록 반환
	 */
	 ...
	 * @return int - 삭제 요청 처리 실패한 게시글 번호 or 0(삭제 성공 시)
	 */
	public int boardDelete(HttpServletRequest request) {
		int qnaNum = Integer.parseInt(request.getParameter("qnaNum"));
		boolean isDelete = dao.boardDelete(qnaNum);
		return isDelete ? 0 : qnaNum;
	}
	...
	/**
	 * @param request - 조회수를 증가시킬 게시글 번호
	 * @return int - 게시글 번호
	 */
