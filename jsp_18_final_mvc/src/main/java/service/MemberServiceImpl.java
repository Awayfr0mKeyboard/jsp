...
public class MemberServiceImpl implements MemberService {
	...
	public boolean memberLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		if(member != null) {
			...
			if(login != null) {
				...
				cookie.setPath("/");		// 현재 서버에서 쿠키를 어떤 경로에서 사용 허용할 것인지 지정
		...
		try {
			...
			if(member != null && member.getPass().equals(tempPass)) {
				...
				if (isDeleted) {
					...
					print.println("alert('회원탈퇴 성공!');");
					print.println("location.href='test';");					
				} else {
					print.println("alert('정상적으로 처리 못했습니다. 다시 요청해주세요.');");
					print.println("history.back();");
				}
			} else {
				// 로그인되어 있지 않거나 비밀번호가 일치하지 않음.
	...
	public void logOut(HttpServletRequest request, HttpServletResponse response) {
		// pageContext(Servlet 에는 존재하지 않음) < request < session < application
		HttpSession session = request.getSession();
		// session.removeAttribute("member"); // 세션에 등록된 인증된 회원 정보 제거
		session.invalidate();  // session 무효화
		
		// 자동 로그인이 등록되어 있다면 쿠키 정보도 제거
